...
| Project | Description |
| --- | --- |
| [`src/ProcessKillRepro`](src/ProcessKillRepro) | Runs the following 10 times in a row: starts `/bin/sleep 60` with `Process.Start`, calls `Process.Kill(entireProcessTree: true)`, waits for the sleeping child to exit and reports its exit code. |
